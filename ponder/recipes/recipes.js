...
function renderRecipes(recipeLegion) {

    const recipeList = document.getElementById('recipebox');
    recipeList.innerHTML = '';

    recipeLegion.forEach(recipe => {
        const article = document.createElement('article');
        article.className = 'recipes recipebox';

        let html = `
                <img src=${recipe.image} alt=${recipe.name} class="recipeimg">
                <div class="recipetext">
                <div class="recipetype">${tagTemplate(recipe.tags)}</div>
                <h2>${recipe.name}</h2>
                <p>${starTemplate(recipe.rating)}</p>
...
function search() {
    ...
    let recipeQuery = document.querySelector('input').value.toLowerCase();
